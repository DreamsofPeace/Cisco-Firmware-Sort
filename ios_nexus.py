...
def fileprocessornxos (filename):
    logging.debug("Module: ios_nexus")
    logging.debug("Sub:    fileprocessornxos")
    ...
    filename.startswith("Nexus1000V")
        ...
        prodname = product("n1000v")
        fileprocnxos1000v (filename,prodname)
    ...
    splitbydot[0] == "nxos64-msll"
        ...
        if filename.startswith("n3000-uk9-kickstart."):
            ...
            utils_dev_v2_vf_imagecode (filename,prodname,imagecode,workname)
...
def nexus7ksliceandice (filename,prodname,supcode):
    logging.debug("Sub:    nexus7ksliceandice")
    splitbydot = filename.split(".")
...
def fileprocnxosfivedigit (filename,prodname,imagecode):
    logging.debug("Sub:    fileprocnxosfivedigit")
    splitbydot = filename.split(".")
    nxosver = util2digit (splitbydot[1],splitbydot[2])
...
def fileprocnxos1000v (filename,prodname):
    logging.debug("Sub:    fileprocnxos1000v")
    if (
...
def fileprocessornxos9ksmu (filename,prodname,imagecode):
    logging.debug("Sub:    fileprocessornxos9ksmu")
