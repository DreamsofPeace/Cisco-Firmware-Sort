...
import logging
import re
from iosutils import product, util_dot_join, filepath, filemove
...
def fileprocessor_wireless (filename):
    logging.debug("Module: ios_wireless")
    logging.debug("Sub:    fileprocessor_wireless")
    ...
    filename.startswith("AP_BUNDLE_CTVM_")
        ...
        wireless_controller_virtual (filename,prodname)
    ...
    filename.startswith("AIR-AP1815-K9-ME-") 
        ...
        wireless_all_dash (filename,prodname,workname)
    ...
    filename.startswith("AIR-AP1830-K9-") 
        ...
        if len(newname) > 2:
            wireless_all_dash_dot (filename,prodname,workname)
        else:
            wireless_all_dash (filename,prodname,workname)
    ...
    filename.startswith("AP_BUNDLE_3500_") 
        ...
        wireless_all_underscore (filename,prodname,workname)
...
def wireless_all_underscore (filename,prodname,workname):
    logging.debug("Sub:    wireless_all_underscore")
    splitbyuscore = workname.split("_")
    vertwo = util_dot_join(splitbyuscore[0],splitbyuscore[1])
    verfour = util_dot_join(splitbyuscore[0],splitbyuscore[1],splitbyuscore[2],splitbyuscore[3])
    filepathjoined = filepath(prodname,vertwo,verfour)
    filemove (filepathjoined, filename)

def wireless_all_dash_dot (filename,prodname,workname):
    logging.debug("Sub:    wireless_all_dash_dot")
    splitbydot = workname.split(".")
    vertwo = util_dot_join(splitbydot[0],splitbydot[1])
    verfour = util_dot_join(splitbydot[0],splitbydot[1],splitbydot[2],splitbydot[3])
    filepathjoined = filepath(prodname,vertwo,verfour)
    filemove (filepathjoined, filename)

def wireless_all_dash (filename,prodname,workname):
    logging.debug("Sub:    wireless_all_dash")
    splitbydash = workname.split("-")
    vertwo = util_dot_join(splitbydash[0],splitbydash[1])
    verfour = util_dot_join(splitbydash[0],splitbydash[1],splitbydash[2],splitbydash[3])
    filepathjoined = filepath(prodname,vertwo,verfour)
    filemove (filepathjoined, filename)

def wireless_controller_virtual (filename,prodname):
    logging.debug("Sub:    wireless_controller_virtual")
    if filename == "AIR-CTVM-K9-1-10-0-0-FUS.aes":
        ...
        vertwo = util_dot_join(filenamesplit[0],filenamesplit[1])
        verfour = util_dot_join(filenamesplit[0],filenamesplit[1],filenamesplit[2],filenamesplit[3])
