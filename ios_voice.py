...
def fileprocessorvoice(filename):
    logging.debug("Module: ios_voice")
    logging.debug("Sub:    fileprocessorvoice")
...
def fileprocphone(filename):
    if filename.startswith("cmterm-3905"):
        ...
        workname = file_proc_strip (filename)
        workname = workname.replace("cmterm-3905.","")
        file_proc_phone_noimage(filename,prodname,workname)
        ...
        file_proc_phone_dash(filename,prodname,imagecode,workname)
...
def file_proc_phone_dash(filename,prodname,imagecode,workname):
    if debug1:
...
def file_proc_phone_noimage(filename,prodname,workname):
    if debug1:
